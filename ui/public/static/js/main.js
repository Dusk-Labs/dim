  ...
  class extends HTMLElement {
    ...
    scroll() {
      return (event) => {
        ...
        const scrollRightPercentage =
          ((this.ul.scrollLeft + this.ul.offsetWidth) / this.size) * 100;
    ...
    scrollPrevious() {
      return (event) => {
        ...
        const elementsToScroll =
          Math.floor(this.ul.offsetWidth / elementWidth) - 1;
        this.ul.scrollLeft =
          this.ul.scrollLeft - elementsToScroll * elementWidth;
        this.scrollHandler();
      };
    }
    ...
    connectedCallback() {
      ...
      this.next.disabled =
        this.ul.scrollLeft >= this.size - this.ul.offsetWidth;
...
customElements.define(
  ...
  class extends HTMLElement {
    constructor() {
      ...
      const dashManifestSource = this.video.querySelector(
        "source[type='application/dash+xml']"
      );
      if (dashManifestSource) {
        this.manifestUri = dashManifestSource.src;
      } else {
        this.manifestUri = this.video.currentSrc;
      }
    }
    ...
    init() {
      return () => {
        ...
        this.player.load(this.manifestUri).then(
          () => {
            // Player loaded
            // const directPlayTrack = this.player
            //   .getVariantTracks()
            //   .filter((t) => t.label === "Direct Play")[0];
            // if (directPlayTrack)
            //   this.player.selectVariantTrack(directPlayTrack);
          },
          (e) => {
...
function initCustomShakaElements() {
   ...
   * @export
   */
  window.shaka.ui.PresentationCurrentTime = class extends (
    window.shaka.ui.Element
  ) {
    /**
    ...
    constructor(parent, controls) {
      ...
      this.container_.classList.add(
        "shaka-elapsed-and-duration-time",
    ...
    updateTime_() {
      ...
      if (seekRangeSize) {
        this.setValue_(
          this.durationTime_,
          this.buildTimeString(seekRangeSize, showHour)
        );
      }
   ...
   * @final
   */
  window.shaka.ui.PresentationCurrentTime.Factory = class {
    /** @override */
    create(rootElement, controls) {
      return new window.shaka.ui.PresentationCurrentTime(rootElement, controls);
    }
  };

  window.shaka.ui.Controls.registerElement(
    "elapsed_and_duration",
    new window.shaka.ui.PresentationCurrentTime.Factory()
  );
}
