...
const SelectUnmatchedMediaFileControls = () => {
  ...
  const selectAll = useCallback(() => {
    const selected = Object.values(selectedFiles).filter(
  ...
  const allFilesInCurrentFolderSelected =
    selected.length === unmatched.items[currentFolder]?.length;

  return (
    <div className="selectUnmatchedMediaFileControls">
      <button
        className={`selectAll active-${allFilesInCurrentFolderSelected}`}
        onClick={selectAll}
      >
        <div className="selectBox">
          <CheckIcon />
        </div>
        {allFilesInCurrentFolderSelected ? "Clear selection" : "Select all"}
      </button>
    </div>
  );
};
