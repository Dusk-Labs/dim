...
const SelectUnmatchedMediaOptions = () => {
  ...
  const {
    ...
    setFilesMatched,
    setMatching,
  } = useContext(SelectUnmatchedContext);
  ...
  const match = useCallback(async () => {
    if (!tmdbID || !mediaType || Object.keys(selectedFiles).length === 0)
      return;
    ...
    for (const file of files) {
      ...
      const req = await fetch(
        `/api/v1/mediafile/${file.id}/match?tmdb_id=${tmdbID}&media_type=${mediaType}`,
        config
      );
    ...
    dispatch(
      addNotification({
        msg: `Sucessfuly matched ${files.length} ${
  ...
  return (
    <div className="selectUnmatchedMediaOptions">
      <Button type="secondary" onClick={() => setShowUnmatched(false)}>
        Close
      </Button>
      <Button
        onClick={() => match()}
