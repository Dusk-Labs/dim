...
pub trait MediaMatcher {
    async fn batch_match(&self, tx: &mut database::Transaction<'_>, provider: Arc<dyn ExternalQuery>, work: Vec<WorkUnit>);
}

pub async fn scan_directory(conn: &mut database::DbConnection, library_id: i64, dirs: Vec<impl AsRef<Path>>) {
    ...
    for unit in mediafiles.into_iter().zip(parsed.into_iter()).map(|(file, (_, meta))| WorkUnit(file, meta)).chunks(128).into_iter() {
        let mut lock = conn.writer().lock_owned().await;
        let mut tx = database::write_tx(&mut lock).await.unwrap();

        matcher.batch_match(&mut tx, provider.clone(), unit.collect()).await;
